...
workflow {
    ...
    try {
        ...
        if (e.toString().contains('MalformedInput') || e.toString().contains('Input length')) {
    ...
    image_overlap_ch = Channel.of(params.image_overlap)
    ...
    shift_ch = Channel.of(params.shift)
    contrast_ch = Channel.of(params.contrast)
    ...
    if (params.DO_SEGMENTATION_MONTAGE) {
        register_flag = updatepaths_result.mix(register_result).collect()
        combined_seg_montage_ch = register_flag
            .combine(experiment_ch).combine(morphology_ch).combine(seg_ch).combine(norm_ch)
            .combine(lower_ch).combine(upper_ch).combine(sd_ch).combine(proximity_radius_ch)
            .combine(well_ch).combine(tp_ch).combine(well_toggle_ch).combine(tp_toggle_ch)
        seg_ch = SEGMENTATION_MONTAGE(combined_seg_montage_ch)
        seg_ch.view { it }
    ...
    if (params.DO_MONTAGE) {
        ...
        combined_montage_ch = montage_flag
            .combine(experiment_ch).combine(tiletype_ch).combine(montage_pattern_ch)
            .combine(well_ch).combine(tp_ch).combine(channel_ch)
            .combine(well_toggle_ch).combine(tp_toggle_ch).combine(channel_toggle_ch).combine(image_overlap_ch)
        montage_ch = MONTAGE(combined_montage_ch)
    ...
    if (params.DO_ALIGN_TILES_DFT) {
        align_tiles_dft_ch = ALIGN_TILES_DFT(
            experiment_ch, morphology_ch, well_ch, tp_ch, channel_ch,
            well_toggle_ch, tp_toggle_ch, channel_toggle_ch, tile_ch, shift_dict_ch
        )
    ...
    if (params.DO_ALIGN_MONTAGE_DFT) {
        combined_align_montage_ch = Channel.of(true)
            .combine(experiment_ch).combine(morphology_ch).combine(well_ch).combine(tp_ch).combine(channel_ch)
            .combine(well_toggle_ch).combine(tp_toggle_ch).combine(channel_toggle_ch).combine(tile_ch).combine(shift_dict_ch)
        align_montage_dft_ch = ALIGN_MONTAGE_DFT(combined_align_montage_ch)
    ...
    if (params.DO_OVERLAY) {
        overlay_ch = OVERLAY(
            track_result, experiment_ch, morphology_ch, well_ch, tp_ch,
            well_toggle_ch, tp_toggle_ch, channel_toggle_ch,
            params.shift, params.contrast, tile_ch
        )
        overlay_ch.view { it }
    }
    ...
    if (params.DO_STD_WORKFLOW) {
        ...
        combined_bundled_std_ch = well_ch
            .combine(experiment_ch).combine(tiletype_ch).combine(montage_pattern_ch)
            .combine(tp_ch).combine(channel_ch).combine(well_toggle_ch).combine(tp_toggle_ch)
            .combine(channel_toggle_ch).combine(image_overlap_ch).combine(morphology_ch)
            .combine(seg_ch).combine(norm_ch).combine(lower_ch).combine(upper_ch).combine(sd_ch)
            .combine(proximity_radius_ch).combine(track_type_ch).combine(distance_threshold_ch)
            .combine(target_channel_ch).combine(shift_ch).combine(contrast_ch).combine(tile_ch)
            .combine(shift_dict_ch).combine(motion_ch).combine(overlay_montage_cell_ids_ch)
            .map { nestedTuple ->
                def flat = nestedTuple.flatten()
                def exp=flat[1]; def tiletype=flat[2]; def montage_pattern=flat[3]
                def chosen_timepoints=flat[4]; def chosen_channels=flat[5]; def wells_toggle=flat[6]
                def timepoints_toggle=flat[7]; def channels_toggle=flat[8]; def image_overlap=flat[9]
                def morphology_channel=flat[10]; def segmentation_method=flat[11]; def img_norm_name=flat[12]
                def lower_area_thresh=flat[13]; def upper_area_thresh=flat[14]; def sd_scale_factor=flat[15]
                def proximity_filter_radius=flat[16]; def track_type=flat[17]; def distance_threshold=flat[18]
                def target_channel=flat[19]; def well=flat[0]; def shift=flat[20]; def contrast=flat[21]
                def tile=flat[22]; def shift_dict=flat[23]; def motion=flat[24]; def cell_ids=flat[25]
        ...
        bundled_std_result_ch.view { tuple ->
            ...
            println "[${bundle_end_timestamp}] COMPLETED... BUNDLED_STD_WORKFLOW for well: $well"
            println "Total time: ${total_time_seconds.round(1)}s (${total_time_minutes.round(2)} min)"
        }
    }
    ...
    if (params.DO_STD_WORKFLOW_IXM) {
        ...
        combined_bundled_ch = well_ch
            ...
            .combine(seg_ch).combine(norm_ch).combine(lower_ch).combine(upper_ch).combine(sd_ch)
            .combine(proximity_radius_ch).combine(track_type_ch).combine(distance_threshold_ch)
            .combine(target_channel_ch).combine(shift_ch).combine(contrast_ch)
            .combine(motion_ch).combine(overlay_montage_cell_ids_ch)
            .map { nestedTuple ->
                def flat = nestedTuple.flatten()
                def exp=flat[1]; def tiletype=flat[2]; def montage_pattern=flat[3]
                def chosen_timepoints=flat[4]; def chosen_channels=flat[5]; def wells_toggle=flat[6]
                ...
                def proximity_filter_radius=flat[16]; def track_type=flat[17]; def distance_threshold=flat[18]
                def target_channel=flat[19]; def well=flat[0]; def shift=flat[20]; def contrast=flat[21]
                def motion=flat[22]; def cell_ids=flat[23]
                return tuple(exp, tiletype, montage_pattern, chosen_timepoints, chosen_channels, wells_toggle,
                           timepoints_toggle, channels_toggle, image_overlap, morphology_channel, segmentation_method,
                           img_norm_name, lower_area_thresh, upper_area_thresh, sd_scale_factor, proximity_filter_radius,
        ...
        bundled_result_ch.view { tuple ->
            ...
            def total_time_seconds = total_time_ms / 1000.0
            def total_time_minutes = total_time_seconds / 60.0
            println "[${bundle_end_timestamp}] COMPLETED... BUNDLED_WORKFLOW_IXM for well: $well"
            println "Total time: ${total_time_seconds.round(1)}s (${total_time_minutes.round(2)} min)"
        }
    }
