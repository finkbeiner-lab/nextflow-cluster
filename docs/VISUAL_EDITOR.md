...
`.../nextflow-cluster/visual-editor/` on the cluster.
...
| TLS cert + key | `/etc/ssl/visual-editor.crt` and `.key` |
| Service account `finkbeiner-svc` | see `deploy/DEPLOYMENT.md` §1.5 |

### Build the image

```bash
cd deepcell            # (== .../nextflow-cluster/visual-editor on the cluster)
( cd frontend && npm ci && npm run build )   # produces frontend/dist
bash deploy/build.sh                          # -> visual-editor.sif + .sha256
```
...
apptainer instance list                       # should list "visual-editor"
```

**Supervised (recommended):**

```bash
sudo cp deploy/visual-editor.service /etc/systemd/system/
sudo systemctl daemon-reload
sudo systemctl enable --now visual-editor
sudo systemctl status visual-editor
```
...
sudo systemctl restart visual-editor
...
journalctl -u visual-editor -f
apptainer instance logs visual-editor          # stdout
apptainer instance logs --err visual-editor    # stderr
```

| Thing | Location |
|---|---|
| Container image | `.../visual-editor/visual-editor.sif` (+ `.sha256`) |
| Editor state (SQLite, workflows, runs) | `EDITOR_DATA` (default `.../visual-editor/data`, `0700`) |
| Nextflow home | `NXF_HOME` (default `.../visual-editor/nxf`) |
...
| `bind() to 0.0.0.0:8080 failed` | Port in use or stale instance → `apptainer instance stop visual-editor` (§4.3) |
